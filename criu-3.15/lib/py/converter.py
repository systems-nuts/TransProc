...
class Converter:  # TODO: Extend the logic for multiple PIDs
    ...
    def __init__(
        ...
        debug,
        no_transform=False,
        measure_transform=False,
    ):
        assert os.path.exists(src_dir), "Source directory does not exist"
        assert os.path.exists(join(src_dir, src_bin)), "Source binary does not exist"
        ...
        img_files = [
            f for f in listdir(src_dir) if (isfile(join(src_dir, f)) and "img" in f)
        ]
        for f in img_files:
            if CGROUP in f:
            ...
            if CORE in f:
                if (
                    CORE in self.src_image_file_paths
                    and CORE in self.dest_image_file_paths
                ):
                    self.src_image_file_paths[CORE].append(join(src_dir, f))
                    self.dest_image_file_paths[CORE].append(join(dest_dir, f))
                elif (
                    CORE not in self.src_image_file_paths
                    and CORE not in self.dest_image_file_paths
                ):
                    self.src_image_file_paths[CORE] = [join(src_dir, f)]
                    self.dest_image_file_paths[CORE] = [join(dest_dir, f)]
                else:
                    raise Exception(
                        "Core file cannot be in source cache but not in dest cache"
                    )
            if FDINFO in f:
    ...
    def get_code_pages_offset(self, mm_img, pm_img):
        ...
        for p in pm_img["entries"][1:]:
            if int(p["vaddr"], 16) >= int(start_vaddr, 16) and int(
                p["vaddr"], 16
            ) <= int(end_vaddr, 16):
    ...
    def remove_region_type(
        self, mm_img, pagemap_img, page_tmp, original_size, region_type
    ):
        region_start = -1
        region_end = -1
        ...
        if found:
            ...
            ret_cnt = page_tmp.read(cnt_size)  # noqa:F841
    ...
    def __add_target_region(
        ...
        for pgmap in pages_list:
            ...
            addr = int(pgmap["vaddr"], 16)
            page_nbr = pgmap["nr_pages"]
            addr_end = addr + (page_nbr * PAGESIZE)  # noqa:F841
    ...
    def add_target_region(
        ...
        return self.__add_target_region(
            mm_img, pagemap_img, page_tmp, original_size, mm_tmpl, pgmap_tmpl, cnt_tmpl
        )
    ...
    def transform_files_file(self):
        ...
        (fid, idx) = self.get_binary_info(
            self.src_image_file_paths[FILES], self.src_image_file_paths[MM]
        )
        (d, b) = self.get_dest_bin_path()
        bin = join(d, b)
    ...
    def transform_target_mem(
        self,
    ):  ##mm, pagemap, pages for vdso, vvar, code_pages and vsyscall
        assert MM in self.src_image_file_paths, "src mm img path not found"
        ...
        vma = [
            v
            for v in dest_mm_img["entries"][0]["vmas"]
            if "PROT_EXEC" in v["prot"] and "VMA_AREA_VDSO" not in v["status"]
        ][0]
        ...
        shutil.copyfile(
            self.src_image_file_paths[PAGES], self.dest_image_file_paths[PAGES]
        )
        orig_size = os.stat(self.dest_image_file_paths[PAGES]).st_size
        dest_pages = open(self.dest_image_file_paths[PAGES], "r+b")

        ret_size = self.remove_region_type(
            dest_mm_img, dest_pm_img, dest_pages, orig_size, "VDSO"
        )
        if ret_size > 0:
            ret_size = self.add_target_region(
                dest_mm_img, dest_pm_img, dest_pages, orig_size, "VDSO"
            )
            if ret_size > 0:
                orig_size = ret_size
        ...
        ret_size = self.remove_region_type(
            dest_mm_img, dest_pm_img, dest_pages, orig_size, "VVAR"
        )
        if ret_size > 0:
            ret_size = self.add_target_region(
                dest_mm_img, dest_pm_img, dest_pages, orig_size, "VVAR"
        ...
        if self.arch == AARCH64:
            ret_size = self.remove_region_type(
                dest_mm_img, dest_pm_img, dest_pages, orig_size, "VSYSCALL"
            )
    ...
    def transform_stack_and_regs(self):  # call after transform_core_file
        for i in range(len(self.src_image_file_paths[CORE])):
            src_core = self.load_image_file(
                self.src_image_file_paths[CORE][i], False, True, False
            )
            dest_core = self.load_image_file(
                self.dest_image_file_paths[CORE][i], False, True, False
            )
            ...
            if self.arch == AARCH64:
                ...
                dest_regset = reg_aarch64.RegsetAarch64(
                    dest_core["entries"][self.entry_num]
                )
                ...
                src_regset = reg_aarch64.RegsetAarch64(
                    src_core["entries"][self.entry_num]
                )
            ...
            unwind_and_size(
                self.src_rewrite_ctx, self.dest_rewrite_ctx, self.transform_needed
            )
            assert len(self.src_rewrite_ctx.activations) == len(
                self.dest_rewrite_ctx.activations
            ), "act count unequal for src and dest"
    ...
    def rewrite_context_init(self, src_handle, src_regset, dest_handle, dest_regset):
        ...
        dest_pages = open(self.dest_image_file_paths[PAGES], "r+b")
        (src_st_top_offset, src_st_base_offset) = self.get_stack_page_offset(
            src_pm, src_sp
        )
        (dest_st_top_offset, dest_st_base_offset) = self.get_stack_page_offset(
            dest_pm, dest_sp
        )
        src_rewrite_ctx = RewriteContext(
            src_handle, src_regset, src_st_top_offset, src_st_base_offset, src_pages
        )
        self.src_rewrite_ctx = src_rewrite_ctx
        dest_rewrite_ctx = RewriteContext(
            dest_handle,
            dest_regset,
            dest_st_top_offset,
            dest_st_base_offset,
...
class X8664Converter(Converter):
    ...
    def assert_conditions(self):  # call before calling recode
        ...
        a_stat = os.stat(x64_bin)  # noqa:F841
        b_stat = os.stat(base)  # noqa:F841
...
class Aarch64Converter(Converter):
    ...
    def assert_conditions(self):  # call before calling recode
        ...
        assert a_stat.st_mode == b_stat.st_mode, (
            "rwx modes do not match for src and dest bin"
        )
    ...
    def get_vdso_template(self):
        ...
        pgmap = {"vaddr": "0xffffacaa6000", "nr_pages": 1, "flags": "PE_PRESENT"}
        dir_path = os.path.dirname(os.path.realpath(__file__))
    ...
    def transform_core_file(self):
        ...
        for i in range(len(self.src_image_file_paths[CORE])):
            ...
            dest_core = copy(src_core)
            dest_tls = (
                int(src_core["entries"][0]["thread_info"]["gpregs"]["fs_base"], 16)
                + 272
            )
            ...
            dest_core["entries"][0]["tc"]["task_state"] = (
                1  # change task state to alive
            )
            self.dump_image_file(self.dest_image_file_paths[CORE][i], dest_core)
            self.log("Core file template created")
